// vietnameseenglish - creat Vietnamese to English dictionary program using existing englishvietnames
package main
...
func main() {
	...
	// create a new map for vietnamese dictionary
	vietnamese := make(map[string]string, len(dict)) // len(dict) is not a required feature.

	// now clone the keys and values pair from the dict by reverse the value to key and save
